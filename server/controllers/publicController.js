...
const getCarsByUser = asyncHandler(async (req, res, next) => {
  ...
  if (select) {
    ...
    // Default safe fields
    const publicFields = ['_id', 'brand', 'model', 'year', 'color', 'category', 'fuelType', 'fuelConsumption', 'engine', 'transmission', 'seats', 'doors', 'description', 'pricing', 'mileageLimits', 'location', 'features', 'images', 'equipment', 'badges', 'status'];
    responseData = paginatedCars.map(car => {
      const safeData = {};
...
const createReservationByUser = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (tenantAdminUser && tenantAdminUser.email.toLowerCase() === 'lerent@lerent.sk') {
    ...
    } else if (tenantAdminUser && tenantAdminUser.email.toLowerCase() === 'nitra-car@nitra-car.sk') {
      ...
      console.log(`🔢 [NITRACAR] Generated reservation number: ${reservationNumber}`);
    } else {
    ...
    const populatedReservation = await Reservation.findById(savedReservation._id)
      ...
      .populate('car', 'brand model year registrationNumber images mileageLimits pricing deposit')
...
const createPublicReservation = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (tenantAdminUserForNumber && tenantAdminUserForNumber.email.toLowerCase() === 'lerent@lerent.sk') {
      ...
      reservationNumber = `${year}${month}${sequentialNumber}`;
      console.log(`🔢 [LERENT] Generated reservation number: ${reservationNumber} (Year: ${year}, Month: ${month}, #${sequentialNumber})`);
    } else if (tenantAdminUserForNumber && tenantAdminUserForNumber.email.toLowerCase() === 'nitra-car@nitra-car.sk') {
      // NitraCar format: YYYYNNN (year + sequential number within year)
      const now = new Date();
      const year = now.getFullYear();
      const yearPrefix = `${year}`;
    ...
    const populatedReservation = await Reservation.findById(savedReservation._id)
      .populate('customer', 'firstName lastName email phone')
      .populate('car', 'brand model year registrationNumber dailyRate images mileageLimits pricing deposit');
