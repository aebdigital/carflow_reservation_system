...
const confirmReservation = asyncHandler(async (req, res, next) => {
  const { notes, skipEmail, skipPaymentInfo } = req.body;

  // For NitraCar: skipPaymentInfo=true means "Potvrdiť bez zálohy" (email sent but without payment info)
  // Legacy support: skipEmail is now treated as skipPaymentInfo
  const shouldSkipPaymentInfo = skipPaymentInfo === true || skipEmail === true;
...
const generateSlovakAgreement = asyncHandler(async (req, res, next) => {
  try {
    ...
    const reservation = await Reservation.findOne({
      ...
      .populate('customer', 'firstName lastName email phone address licenseNumber idNumber rodneCislo')
